...
```text
这是一面垂直岩壁，高3米。/这是一面Spray Wall攀岩墙，高5米，仰角18度。
这是一面高3米的攀岩墙，由两块三角形的岩壁组成。右侧岩壁有15度仰角，岩点编号从1到70；左侧岩壁有20度仰角，岩点编号从71到149。

你是一名专业的攀岩定线员，很严谨，定线的时候请考虑到我提到的每一个要求，都很重要。请为身高140cm的儿童设计一系列难度为 **V0-V2** 的攀岩路线18条，在每条线路中练习特定技巧，以技巧名作为线路名。白色横线以下的点不可以做为手点。beta内容为分析每条线路有针对性的帮助她在攀爬过程中练习了哪些特定技巧，怎样指导她才能更大的发挥这些线路的作用，不要出现点的编号。

请**严格按照**以下JSON格式输出你的定线方案，routeName里只写线路名不写难度，routeName里不要出现中文，author写Gemini 2.5 Pro，直接给出JSON代码块：
